...
var registry = []Operation{
	...
	{ID: "message.inbox", Category: "Messages", Summary: "List routed messages", Description: "Lists messages routed to the selected agent without advancing any read cursor. The inbox is an attention surface, so the agent's own messages are excluded unless include_self is set; they remain in topic history, thread, peek, search, receipts, observe, and export. Thread summaries collapse to the earliest still-visible message of each thread, so an incoming reply still appears in a thread the reader started.", CLI: "inbox [--unread] [--threads] [--include-self] [--limit N] [--cursor CURSOR]", HTTP: route("GET", "/v1/inbox"), MCPName: "comms_inbox", RequiresIdentity: true, Parameters: append([]Parameter{
	...
	{ID: "message.wait", Category: "Messages", Summary: "Wait for matching incoming messages", Description: "Blocks until at least one unread message routed to the selected agent matches the optional author and thread filters, then returns that bounded batch with an after cursor for the next call. Preexisting unread matches return immediately, so a caller never sleeps and polls. Any message ID names its thread and is normalized to the thread root; author handles are resolved to stable IDs once, before waiting. Waiting is a read: it does not mark messages read, advance a read-through cursor, send or auto-reply, or promise the recipient acted. Own messages do not satisfy a wait unless include_self is set. Bounded timeout defaults to 30s and must not exceed 1h. Exit behavior: matching messages exit 0, timeout and cancellation both exit 5, invalid arguments exit 2.", CLI: "wait [--from AGENT] [--thread MESSAGE_ID] [--after CURSOR] [--include-self] [--limit N] [--timeout DURATION]", HTTP: route("GET", "/v1/wait"), MCPName: "comms_wait", RequiresIdentity: true, Parameters: []Parameter{
		...
		p("limit", "Maximum number of matching messages to return (default 50, maximum 200).", "integer", QueryParameter, false),
		waitTimeoutParameter(),
	}},
...
func pagingParameters() []Parameter {
	return []Parameter{
		p("limit", "Maximum number of results to return (default 50, maximum 200).", "integer", QueryParameter, false),
