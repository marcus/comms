...
var registry = []Operation{
	...
	{ID: "agent.join", Category: "Agents", Summary: "Join or reconnect a session", Description: "Creates an addressable logical session, or reconnects one by external reference. Without --context or COMMS_CONTEXT the CLI stores the identity per terminal session (COMMS_SESSION, else the tmux pane), falling back to one machine-wide file; it refuses to switch such an implicit identity to a different active agent unless --replace is given.", CLI: "join [HANDLE] [--display-name TEXT] [--purpose TEXT] [--harness NAME] [--project NAME] [--session-ref REF] [--external-namespace NAME --external-key KEY] [--context PATH] [--replace]", HTTP: route("POST", "/v1/agents/join"), MCPName: "comms_join", Mutating: true, Parameters: []Parameter{
	...
	{ID: "agent.wait", Category: "Agents", Summary: "Wait for an agent handle to join", Description: "Blocks until the reference resolves to an active agent and returns it; an already registered agent returns immediately. The predicate is logical registration in Comms only: it does not assert that the session's process is alive, idle, has read a message, or will answer. It never creates, renames, retires, or takes over an identity, and sends or queues nothing. The wait is always bounded. A missing agent ends as timeout, a caller that goes away ends as canceled, and both use exit 5; a reference that resolves to a retired agent is a conflict rather than an endless wait.", CLI: "agent wait AGENT [--timeout DURATION]", HTTP: route("GET", "/v1/agents/{agent}/wait"), MCPName: "comms_wait_agent", Parameters: []Parameter{
		p("agent", "Stable agent ID or expected friendly handle.", "string", PathParameter, true),
		waitTimeoutParameter(),
	}},
	...
	{ID: "message.inbox", Category: "Messages", Summary: "List routed messages", Description: "Lists messages routed to the selected agent without advancing any read cursor. Bodies are previews by default, cut to the lead paragraph and at most 160 characters and marked body_truncated; pass full to get complete bodies. Triage from previews and peek or thread what you intend to act on. The inbox is an attention surface, so the agent's own messages are excluded unless include_self is set; they remain in topic history, thread, peek, search, receipts, observe, and export. Thread summaries collapse to the earliest still-visible message of each thread, so an incoming reply still appears in a thread the reader started.", CLI: "inbox [--unread] [--threads] [--include-self] [--full] [--limit N] [--cursor CURSOR]", HTTP: route("GET", "/v1/inbox"), MCPName: "comms_inbox", RequiresIdentity: true, Parameters: []Parameter{
		...
		p("include_self", "Include the selected agent's own messages, which are excluded by default.", "boolean", QueryParameter, false),
		p("full", "Return complete message bodies instead of previews.", "boolean", QueryParameter, false),
		p("limit", "Maximum number of results to return (default 20, maximum 500).", "integer", QueryParameter, false),
		p("cursor", "Opaque continuation cursor, distinct from a subscription read cursor.", "string", QueryParameter, false),
	}},
	{ID: "message.wait", Category: "Messages", Summary: "Wait for matching incoming messages", Description: "Blocks until at least one unread message routed to the selected agent matches the optional author and thread filters, then returns that bounded batch with an after cursor for the next call. Preexisting unread matches return immediately, so a caller never sleeps and polls. Any message ID names its thread and is normalized to the thread root; author handles are resolved to stable IDs once, before waiting. Waiting is a read: it does not mark messages read, advance a read-through cursor, send or auto-reply, or promise the recipient acted. Own messages do not satisfy a wait unless include_self is set. Bounded timeout defaults to 30s and must not exceed 1h. Exit behavior: matching messages exit 0, timeout and cancellation both exit 5, invalid arguments exit 2.", CLI: "wait [--from AGENT] [--thread MESSAGE_ID] [--after CURSOR] [--include-self] [--limit N] [--timeout DURATION]", HTTP: route("GET", "/v1/wait"), MCPName: "comms_wait", RequiresIdentity: true, Parameters: []Parameter{
		p("from", "Only match messages written by this agent ID or handle.", "string", QueryParameter, false),
		p("thread", "Only match messages in the thread containing this message ID.", "string", QueryParameter, false),
		p("after", "Opaque continuation cursor from a previous wait; distinct from a subscription read cursor.", "string", QueryParameter, false),
		p("include_self", "Let the selected agent's own messages satisfy the wait.", "boolean", QueryParameter, false),
		p("limit", "Maximum number of matching messages to return (default 50, maximum 500).", "integer", QueryParameter, false),
		waitTimeoutParameter(),
	}},
	{ID: "message.topic", Category: "Messages", Summary: "Read a topic", Description: "Lists visible messages in one topic without advancing a cursor. Defaults to ascending sequence order from the beginning of history; --latest returns the most recent message window. Cursors are opaque sequence tokens that continue in the requested direction.", CLI: "topic messages TOPIC [--latest] [--limit N] [--cursor CURSOR]", HTTP: route("GET", "/v1/topics/{topic}/messages"), MCPName: "comms_topic_messages", Parameters: append([]Parameter{
		p("topic", "Stable topic ID or current name.", "string", PathParameter, true),
		p("latest", "Return the most recent visible messages window instead of starting from the oldest.", "boolean", QueryParameter, false),
	}, pagingParameters()...)},
	{ID: "message.thread", Category: "Messages", Summary: "Read a message thread", Description: "Returns a thread with live descendants and retained expired ancestors needed for context. Defaults to ascending sequence order; --latest returns the most recent message window. Cursors are opaque sequence tokens that continue in the requested direction.", CLI: "thread MESSAGE_ID [--latest] [--limit N] [--cursor CURSOR]", HTTP: route("GET", "/v1/messages/{message}/thread"), MCPName: "comms_thread", Parameters: append([]Parameter{
		p("message", "Any message ID in the thread.", "string", PathParameter, true),
		p("latest", "Return the most recent visible messages window instead of starting from the oldest.", "boolean", QueryParameter, false),
	}, pagingParameters()...)},
	...
	{ID: "message.receipts", Category: "Messages", Summary: "Inspect cursor acknowledgments and retrieval activity", Description: "Reports two independent facts per agent. State is the cursor fact: read means the subscriber advanced its cursor through this message. Retrieval is what actually reached the agent: inspected_at means a full body was returned to it, seen_at without inspected_at means it only saw an inbox preview, and seen_count counts return visits. Subscribers lists relevant subscribers; inspectors lists identified readers that are not subscribed, such as an orchestrator that peeked. Retrieval is not an acknowledgment.", CLI: "receipts MESSAGE_ID [--detailed]", HTTP: route("GET", "/v1/messages/{message}/receipts"), MCPName: "comms_receipts", Parameters: []Parameter{p("message", "Message ID.", "string", PathParameter, true)}},
...
func waitTimeoutParameter() Parameter {
	return p("timeout", "Bounded wait duration such as 30s or 2m. Defaults to 30s and must not exceed 1h; there is no unbounded mode.", "string", QueryParameter, false)
}

func pagingParameters() []Parameter {
	return []Parameter{
		p("limit", "Maximum number of results to return (default 50, maximum 500).", "integer", QueryParameter, false),
		p("cursor", "Opaque continuation cursor, distinct from a subscription read cursor.", "string", QueryParameter, false),
	}
}
