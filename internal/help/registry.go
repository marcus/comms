...
var registry = []Operation{
	...
	{ID: "agent.join", Category: "Agents", Summary: "Join or reconnect a session", Description: "Creates an addressable logical session, or reconnects one by external reference.", CLI: "join [HANDLE] [--display-name TEXT] [--purpose TEXT] [--harness NAME] [--external-namespace NAME --external-key KEY] [--context PATH]", HTTP: route("POST", "/v1/agents/join"), MCPName: "comms_join", Mutating: true, Parameters: []Parameter{
	...
	{ID: "topic.create", Category: "Topics and subscriptions", Summary: "Create a public topic", Description: "Creates a named, discoverable public topic.", CLI: "topic create NAME", HTTP: route("POST", "/v1/topics"), MCPName: "comms_create_topic", Mutating: true, Parameters: []Parameter{
	...
	{ID: "topic.ensure", Category: "Topics and subscriptions", Summary: "Ensure an integration topic", Description: "Finds or creates the one topic associated with an integration-owned external reference.", CLI: "topic ensure --external-namespace NAME --external-key KEY --name NAME", HTTP: route("PUT", "/v1/topics/by-external-reference"), MCPName: "comms_ensure_topic", Mutating: true, Parameters: []Parameter{
	...
	{ID: "message.publish", Category: "Messages", Summary: "Publish a topic message", Description: "Appends a titled root message to one topic.", CLI: "publish TOPIC --title TEXT [--body TEXT | --body-file PATH | -]", HTTP: route("POST", "/v1/messages"), MCPName: "comms_publish", Mutating: true, RequiresIdentity: true, Parameters: messageParameters(true, true)},
	{ID: "message.direct_send", Category: "Messages", Summary: "Send a direct message", Description: "Ensures a two-member direct topic and publishes a root message to it.", CLI: "send @AGENT --title TEXT [--body TEXT | --body-file PATH | -]", HTTP: route("POST", "/v1/direct-messages"), MCPName: "comms_send", Mutating: true, RequiresIdentity: true, Parameters: append([]Parameter{p("agent", "Recipient stable ID or current handle.", "string", BodyParameter, true)}, messageContentParameters(true)...)},
	{ID: "message.reply", Category: "Messages", Summary: "Reply in a thread", Description: "Appends a reply in the same topic and thread as its parent.", CLI: "reply MESSAGE_ID [--title TEXT] [--body TEXT | --body-file PATH | -]", HTTP: route("POST", "/v1/messages/{message}/replies"), MCPName: "comms_reply", Mutating: true, RequiresIdentity: true, Parameters: append([]Parameter{p("message", "Parent message ID.", "string", PathParameter, true)}, messageContentParameters(false)...)},
