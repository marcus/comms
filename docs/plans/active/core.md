...
10. **Generated agent help:** one operation/capability registry drives CLI help, versioned JSON descriptions, HTTP OpenAPI, MCP tool descriptions, and agent onboarding information. `comms instructions` describes capabilities, guarantees, and optional usage patterns without prescribing polling, startup, or handoff behavior. Static `AGENTS.md` text only tells an agent how to discover those instructions.
